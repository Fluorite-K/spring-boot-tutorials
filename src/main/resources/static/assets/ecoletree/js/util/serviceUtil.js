	...
	ETService.prototype = {
		...
		__private : (function() {
			...
			var setErrorPage = function() {
				var gotoURL = getContextPath() + '/' + getLang(true);
				...
				header += getContextPath() + '/' + getLang(true) + '"><img src="';
			...
			var submitForm = function(url, method, postData) {
				...
				$form = $("<form />").prop("method", method).prop("action", getContextPath() + "/" + getLang(true) + url).hide().appendTo("body"); // IE는 form이 화면에 있어야 submit 가능
